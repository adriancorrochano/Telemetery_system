...
class ImuManager {
  ...
  public:
    ...
    void init() {
      ...
      //we set sensor range to 8G so it doesn't max out on bumps
      mpu.setAccelerometerRange(MPU6050_RANGE_8_G);
      //we the filter small vibrations (21Hz bandwidth)
    ...
    void update() {
      ...
      // Simple threshold: if force is huge (like > 3.5G), we then assume a crash
      if (gForce > 3.5) 
        crashDetected = true; 
      else 
        crashDetected = false;
